...
var (
	...
	blockReward   = new(big.Int).Mul(new(big.Int).SetUint64(750), big.NewInt(params.Ether)) // Block reward 750
	blockInterval = uint64(10368000)                                                           // The validators rewards is halved for every 10368000 blocks.
)
...
func (c *Congress) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) error {
	...
	// Give rewards to validator for each block produced.
	if err := c.validatorRewards(chain, header, state, len(txs)); err != nil {
		panic(err)
	}
...
func (c *Congress) validatorRewards(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txCount int) error {
	// rewards = blockReward / 2 ^ (blockNumber / blockInterval)
	rewards := new(big.Int).Div(blockReward, new(big.Int).SetUint64(uint64(math.Pow(2, float64(header.Number.Uint64()/blockInterval)))))
	...
	if txCount > 0 {
		...
		if fee.Cmp(common.Big0) > 0 {
			...
			rewards = new(big.Int).Add(rewards, fee)
		}
	}

	log.Info("Validator block rewards", "number", header.Number, "rewards", rewards)
	...
	msg := types.NewMessage(header.Coinbase, &validatorsContractAddr, nonce, rewards, math.MaxUint64, new(big.Int), data, true)

	if _, err := executeMsg(msg, state, header, newChainContext(chain, c), c.chainConfig); err != nil {
		log.Error("ExecuteMsg failed", "err", err)
		return err
	}
